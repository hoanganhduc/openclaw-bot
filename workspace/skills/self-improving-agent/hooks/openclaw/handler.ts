...
function parsePendingSummary(): PendingSummary {
...
function buildBootstrapReminder(summary: PendingSummary): string {
...
const handler: HookHandler = async (event) => {
  ...
  if (event.type === 'agent' && event.action === 'bootstrap') {
    ...
    let summary: PendingSummary = { total: 0, high: [], recent: [] };
    try {
